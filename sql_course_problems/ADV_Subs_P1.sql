...
SUBQUERIES_Problem Statement_One

OBJECTIVE:  Identify the top 5 skills that are most frequently mentioned in job postings. 
...
Hint:  Focus on creating a subquery that identifies and 
ranks (ORDER BY in descending order) the top 5 skill IDs 
...
INNER JOIN skills_dim ON sub_sc.skill_id = skills_dim.skill_id
ORDER BY sub_sc.skill_count DESC;
